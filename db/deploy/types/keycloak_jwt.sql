...
comment on column connectivity_intake_public.keycloak_jwt.jti is
  'OPTIONAL - The "jti" (JWT ID) claim provides a unique identifier for the JWT.
   ...
   sensitive string.';
comment on column connectivity_intake_public.keycloak_jwt.exp is
  'REQUIRED - The "exp" (expiration time) claim identifies the expiration time on
   or after which the JWT MUST NOT be accepted for processing.  The
   processing of the "exp" claim requires that the current date/time
   MUST be before the expiration date/time listed in the "exp" claim.';
comment on column connectivity_intake_public.keycloak_jwt.nbf is
  'OPTIONAL - The "nbf" (not before) claim identifies the time before which the JWT
   ...
   NumericDate value.';
comment on column connectivity_intake_public.keycloak_jwt.iat is
...
comment on column connectivity_intake_public.keycloak_jwt.iss is 
  'REQUIRED - The "iss" (issuer) claim identifies the principal that issued the
   JWT.  The processing of this claim is generally application specific.
   The "iss" value is a case-sensitive string containing a StringOrURI
   value.';
comment on column connectivity_intake_public.keycloak_jwt.aud is
...
comment on column connectivity_intake_public.keycloak_jwt.sub is
  'REQUIRED - The "sub" (subject) claim identifies the principal that is the
   ...
   value.';
comment on column connectivity_intake_public.keycloak_jwt.typ is
...
comment on column connectivity_intake_public.keycloak_jwt.azp is
...
comment on column connectivity_intake_public.keycloak_jwt.auth_time is
...
comment on column connectivity_intake_public.keycloak_jwt.session_state is
...
comment on column connectivity_intake_public.keycloak_jwt.acr is
  ...
  string.';
comment on column connectivity_intake_public.keycloak_jwt.email_verified is
  'Whether or not a users email is verified with the login provider.';
comment on column connectivity_intake_public.keycloak_jwt.name is
  'Users full name by combining given_name and family_name set and returned by login provider.';
comment on column connectivity_intake_public.keycloak_jwt.preferred_username is
  'Username as set and returned by provider including provider name separated by @. eg. username@github';
comment on column connectivity_intake_public.keycloak_jwt.given_name is
  'First name as set and returned by login provider.';
comment on column connectivity_intake_public.keycloak_jwt.family_name is
  'Last name as set and returned by login provider.';
comment on column connectivity_intake_public.keycloak_jwt.email is
  'Email address as set and returned by login provider.';
comment on column connectivity_intake_public.keycloak_jwt.broker_session_id is
