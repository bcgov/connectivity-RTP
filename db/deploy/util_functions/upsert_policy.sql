-- Deploy connectivity-intake:util_functions/upsert_policy to pg
...
-- example: select connectivity_intake_public.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'true');
create or replace function connectivity_intake_public.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
  role_name text,
  using_check_statement text,
  table_schema_name text default 'connectivity_intake_public'
)
returns void
as
$function$
...
-- example: select connectivity_intake_public.upsert_policy('test_policy', 'test_table', 'update', 'admin', 'using(true)', 'with check(false)');
create or replace function connectivity_intake_public.upsert_policy(
  policy_name text,
  table_name text,
  operation text,
...
comment on function connectivity_intake_public.upsert_policy(text, text, text, text, text, text) is
  'A generic function for creating or altering row-level security policies';
comment on function connectivity_intake_public.upsert_policy(text, text, text, text, text, text, text) is
