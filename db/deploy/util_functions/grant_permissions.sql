-- Deploy connectivity-intake:util_functions/grant_permissions to pg
-- requires: create_roles

begin;

-- Grants permission for all columns in a table
create or replace function connectivity_intake_public.grant_permissions(
  operation text,
  table_name text,
  role_name text,
  table_schema text default 'connectivity_intake_public'
)
returns void
as
$function$
...
comment on function connectivity_intake_public.grant_permissions(text, text, text, text) is
...
comment on function connectivity_intake_public.grant_permissions(text, text, text, text[], text) is
