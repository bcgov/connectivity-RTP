...
export default function home({ formIndex, formData, validPage, prevPageUrl }) {
  ...
  const pageTitle = (schema, formIndex) => {
    const titles = Object.values(schema.properties).reduce(
      (prev: Array<String>, { title }) => {
        return [...prev, title];
      },
      []
    );
  ...
  const renderDesc = (formIndex) => {
    ...
    if (formIndex === 5) {
      return (
        <div>
          <p>
            This section focuses on existing backbone infrastructure. If this is
            not your area of expertise, skip to the next section. Existing
            backbone is defined as transport that connects to an existing point
            of present (“PoP”) in the community. New backbone is defined by
            requiring fibre or microwave transport to access a PoP in another
            community to support your last mile project.
          </p>
        </div>
      );
    }
    if (formIndex === 6) {
      return (
        <div>
          <p>
            This section is for local and regional governments and First Nations
            to offer additional feedback on internet connectivity in their
            communities.
          </p>
        </div>
      );
    ...
    if (formIndex === 7) {
      return (
        <div>
          <p>
            This section provides an opportunity to add internet connectivity
            infrastructure information that will help meet the objectives
            outlined in the RTP document.
          </p>
        </div>
      );
    }
    if (formIndex === 8) {
      return (
        <div>
          <p>
            This section is to provide feedback on the gaps in highway cellular
            coverage in the province. The Province has identified gaps in
            cellular service along highways (with power) in B.C. The gaps are
            highlighted in the KMZ file for Highway Cellular. Information on how
            to download the KMZ files is linked in Appendix D.
          </p>
        </div>
      );
    }
      ...
      return (
        <div>
          <p>
            ...
            areas where the specified technology is most viable and suitable.
